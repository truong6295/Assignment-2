...
public class Student_Test_TakenController {
  ...
  //creat
    ...
    public ResponseEntity<?> creatStudentTetsTaken(String registrationId,String dateTestTaken,String otherDetails,String testResult, UriComponentsBuilder ucBuilder) throws ParseException{
        ...
        headers.setLocation(ucBuilder.path("/api/studentTetsTaken/").buildAndExpand(studentTetsTakens.getStudentCourseEnrolment().getRegistrationId()).toUri());
        return new ResponseEntity<String>(headers, HttpStatus.CREATED);
    }
  //update
    @RequestMapping(value = "/studentTetsTaken/", method = RequestMethod.PUT)
    public ResponseEntity<?> updateStudentTetsTaken(String registrationId,String dateTestTaken,String otherDetails,String testResult) throws ParseException{
    	Student_Tests_Taken currentStudentTetsTakens = studentTetsTakenService.update(formatHHmm.parse(dateTestTaken), otherDetails, testResult,Integer.parseInt(registrationId));
        return new ResponseEntity<Student_Tests_Taken>(currentStudentTetsTakens, HttpStatus.OK);
    }
  //delete 1 entity
    @RequestMapping(value = "/studentTetsTaken/{id}", method = RequestMethod.DELETE)
    public ResponseEntity<?> deleteStudentTetsTaken(@PathVariable("id") int Id) {
        if(studentTetsTakenService.delete(Id)) {
        	
        	return new ResponseEntity<Student_Tests_Taken>(HttpStatus.NO_CONTENT);
        }
        else
